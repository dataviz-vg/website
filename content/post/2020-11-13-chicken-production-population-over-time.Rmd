...
fls <- tempfile(fileext = ".zip")
  ...
  meat_over_year_chicken <- readr::read_csv(
    file.path(
  ...
  comparison_meat <- meat_over_year_chicken %>%
    left_join(
      worldwide_pop, 
      by = "Year"
    ) %>%
...
source_table <- readr::read_csv(params$table) %>%
  filter(Year %% 5 == 0) %>%
  mutate(
    Chicken = Chicken * 1000
  ) %>%
  # Computing the number of kilogram by human
  mutate(
    one_man_has = Chicken / Human
  )
...
For example, in `r source_table$Year[1]`, the volume of meat produced worldwide was `r source_table$Chicken[1] %>% format()`, and the worldwide population `r source_table$Human[1] %>% format()`, making an estimate of `r source_table$one_man_has[1] %>% format()` kilogram of meat by human on earth. 

In `r source_table$Year[nrow(source_table)]`, the volume of meat produced worldwide was `r source_table$Chicken[nrow(source_table)] %>% format()`, and the worldwide population `r source_table$Human[nrow(source_table)] %>% format()`, making an estimate of `r source_table$one_man_has[nrow(source_table)] %>% format()` kilogram of meat by human on earth. 


```{r meat-over-years-by-human, fig.dim = c(12, 8)}
# VIZ
...
source_table %>%
  ...
  geom_emoji(emoji="1f424", size = 0.1) + 
  geom_text(
    nudge_y = 1,
    size = 6,
    data = source_table, 
    aes(
      y = one_man_has, 
      label = round(one_man_has)
    )
  ) + 
  labs(
    title = "How many chickens is produced during \na given year for one human? 1965-2015",
